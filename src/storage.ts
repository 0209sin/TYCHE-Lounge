import {initialProfile,reduceProfile,validateProfile,type Action,type Profile} from './economy.ts';
...
export async function readProfile():Promise<Profile>{
 const db=await openDatabase();return new Promise((resolve,reject)=>{const req=db.transaction('save').objectStore('save').get('profile');req.onsuccess=()=>{try{resolve(req.result?validateProfile(req.result):initialProfile());}catch(e){reject(e);}};req.onerror=()=>reject(req.error);});
}
export async function changeProfile(action:Action|{type:'import';profile:Profile}):Promise<Profile>{
 const db=await openDatabase();return new Promise((resolve,reject)=>{
  ...
  req.onsuccess=()=>{try{const old=req.result?validateProfile(req.result):initialProfile();if(action.type==='import'){if(old.pending.length)throw new Error('공이 모두 도착한 뒤 복원해 주세요.');result=validateProfile(action.profile,true);result.savedAt=Date.now();}else result=reduceProfile(old,action);store.put(result,'profile');}catch(e){failure=e;tx.abort();}};
